...
extension String {
	// Extracts the first box id code found in a label of meat box,
    // it is a returning
	// the range of the number and the number itself as a tuple.
	// Returns nil if no number is found.
	func extractBoxId() -> (Range<String.Index>, String)? {
		...
		// Potential box id found. Strip out punctuation, whitespace.
        var boxIdDigits = ""
        ...
        do {
            ...
            if let match = regex.firstMatch(in: substring, options: [], range: nsrange) {
                for rangeInd in 1 ..< match.numberOfRanges {
                    ...
                    boxIdDigits += matchString as String
                }
            }
        } catch {
        ...
        // Must be exactly 6 word characters
		guard boxIdDigits.count == 6 else {
			return nil
		}
